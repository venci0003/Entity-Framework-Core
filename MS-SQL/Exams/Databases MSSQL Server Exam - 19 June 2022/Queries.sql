...
SELECT [Name],[at].AnimalType,FORMAT(BirthDate, 'dd.MM.yyyy') FROM Animals AS a
...
SELECT v.[Name],v.PhoneNumber,SUBSTRING(v.[Address],CHARINDEX(',',v.[Address]) + 2,LEN(v.[Address])) AS [Address] FROM Volunteers AS v
...
JOIN VolunteersDepartments AS vp ON vp.Id = v.DepartmentId
